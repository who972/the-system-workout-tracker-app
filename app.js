...
function logCustomWorkout(name, duration, intensity) {
  ...
  // Distribute stats randomly across two stats
  const statKeys = ['str', 'end', 'agi', 'vit'];
  const stat1 = statKeys[Math.floor(Math.random() * 4)];
  let stat2 = statKeys[Math.floor(Math.random() * 4)];
  while (stat2 === stat1) stat2 = statKeys[Math.floor(Math.random() * 4)];
  addStat(stat1, 1);
  addStat(stat2, 1);
...
function renderPlayerCard() {
  ...
  document.getElementById('statEnd').textContent = state.stats.end;
  document.getElementById('statAgi').textContent = state.stats.agi;
  document.getElementById('statVit').textContent = state.stats.vit;
}
...
function logCustomMission(id){const x=getCustomWorkouts().find(v=>v.id===id);if(!x)return;const today=getTodayStr(), key='customMissionDone:'+id+':'+today;if(localStorage.getItem(key)==='true'){alert('SYSTEM: This custom mission is already complete today.');return;}localStorage.setItem(key,'true');addXp(x.xp,'custom mission');addStat(x.stat,2);updateStreak();recordHistory('custom_mission_'+id+'_'+today,x.xp);const hist=workoutHistory();hist.push({date:today,mission:x.name,seconds:x.minutes*60,xp:x.xp,prs:[],sets:[]});localStorage.setItem('systemWorkoutSessions',JSON.stringify(hist.slice(-365)));addSystemMessage(`${x.name} complete — ${x.minutes} min • +${x.xp} XP • +2 ${x.stat.toUpperCase()}`,'quest');saveState();renderAll();if(typeof renderAnalytics==='function')renderAnalytics();alert(`MISSION COMPLETE\n${x.name}\n+${x.xp} XP\n+2 ${x.stat.toUpperCase()}`);}
...
renderSystemMission=function(){_v10RenderSystemMission();const m=SYSTEM_MISSIONS[new Date().getDay()];const cards=document.querySelectorAll('#mission-exercises .mission-exercise');m.exercises.forEach((e,i)=>{const o=getProgressionOverrides()[e[0]];if(o&&cards[i]){const sm=cards[i].querySelector('small');if(sm)sm.textContent=`${e[1]} sets • ${o.target||e[2]}${o.weight?' • '+o.weight+' lb':''}${e[3]?' • '+e[3]+'s rest':''}`;}});}
const _v10RenderLiveSet=renderLiveSet;
renderLiveSet=function(){_v10RenderLiveSet();const m=SYSTEM_MISSIONS[new Date().getDay()],x=flatSets(m)[liveIndex],o=getProgressionOverrides()[x.name];if(o){document.getElementById('liveTarget').textContent=`Target: ${o.target}${o.weight?' @ '+o.weight+' lb':''}`;document.getElementById('liveReps').value=parseTargetReps(o.target)||document.getElementById('liveReps').value;if(o.weight)document.getElementById('liveWeight').value=o.weight;}}
...
function saveCloudConfig(){const url=document.getElementById('cloudUrl').value.trim().replace(/\/$/,'');const key=document.getElementById('cloudAnonKey').value.trim();localStorage.setItem(CLOUD_CONFIG_KEY,JSON.stringify({url,key}));cloudStatus('Cloud configuration saved.',true);}
async function cloudRequest(path,opts={}){const c=getCloudConfig(),s=getCloudSession();if(!c.url||!c.key)throw new Error('Cloud setup is incomplete.');const h={'apikey':c.key,'Content-Type':'application/json',...(opts.headers||{})};if(s?.access_token)h.Authorization='Bearer '+s.access_token;const r=await fetch(c.url+path,{...opts,headers:h});const body=await r.text();let data={};try{data=body?JSON.parse(body):{}}catch(e){}if(!r.ok)throw new Error(data.msg||data.message||data.error_description||'Cloud request failed.');return data;}
async function cloudSignUp(){try{saveCloudConfig();const email=document.getElementById('cloudEmail').value.trim(),password=document.getElementById('cloudPassword').value;if(!email||password.length<6)throw new Error('Enter an email and a password of at least 6 characters.');await cloudRequest('/auth/v1/signup',{method:'POST',body:JSON.stringify({email,password})});cloudStatus('Account created. Check email if confirmation is enabled, then sign in.',true);}catch(e){cloudStatus(e.message)}}
async function cloudSignIn(){try{saveCloudConfig();const email=document.getElementById('cloudEmail').value.trim(),password=document.getElementById('cloudPassword').value;const s=await cloudRequest('/auth/v1/token?grant_type=password',{method:'POST',body:JSON.stringify({email,password})});localStorage.setItem(CLOUD_SESSION_KEY,JSON.stringify(s));cloudStatus('Signed in as '+(s.user?.email||email)+'.',true);renderCloudAccount();}catch(e){cloudStatus(e.message)}}
function cloudSignOut(){localStorage.removeItem(CLOUD_SESSION_KEY);cloudStatus('Signed out.');renderCloudAccount();}
...
async function pullCloudBackup(){try{const s=getCloudSession();if(!s?.user?.id)throw new Error('Sign in first.');cloudStatus('Downloading…');const rows=await cloudRequest('/rest/v1/workout_backups?user_id=eq.'+encodeURIComponent(s.user.id)+'&select=data,updated_at&limit=1');if(!rows.length)throw new Error('No cloud backup found.');if(!confirm('Restore the cloud backup on this device? Current local training data will be replaced.'))return;const keepConfig=localStorage.getItem(CLOUD_CONFIG_KEY),keepSession=localStorage.getItem(CLOUD_SESSION_KEY);localStorage.clear();Object.entries(rows[0].data.localStorage||{}).forEach(([k,v])=>localStorage.setItem(k,v));if(keepConfig)localStorage.setItem(CLOUD_CONFIG_KEY,keepConfig);if(keepSession)localStorage.setItem(CLOUD_SESSION_KEY,keepSession);localStorage.setItem('theSystemLastSync',rows[0].updated_at||new Date().toISOString());location.reload();}catch(e){cloudStatus(e.message)}}
function renderCloudAccount(){const c=getCloudConfig(),s=getCloudSession();const u=document.getElementById('cloudUrl'),k=document.getElementById('cloudAnonKey');if(u&&!u.value)u.value=c.url||'';if(k&&!k.value)k.value=c.key||'';const signed=document.getElementById('cloudSignedIn');if(signed)signed.textContent=s?.user?.email?'SIGNED IN: '+s.user.email.toUpperCase():'NOT SIGNED IN';const last=document.getElementById('lastCloudSync');if(last){const x=localStorage.getItem('theSystemLastSync');last.textContent=x?'Last sync: '+new Date(x).toLocaleString():'No cloud sync yet.';}}
function initCloudSync(){renderCloudAccount();document.getElementById('cloudSaveConfig')?.addEventListener('click',saveCloudConfig);document.getElementById('cloudSignUp')?.addEventListener('click',cloudSignUp);document.getElementById('cloudSignIn')?.addEventListener('click',cloudSignIn);document.getElementById('cloudSignOut')?.addEventListener('click',cloudSignOut);document.getElementById('cloudPush')?.addEventListener('click',pushCloudBackup);document.getElementById('cloudPull')?.addEventListener('click',pullCloudBackup);}
