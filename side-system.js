...
function makeVariants(sq,pu,lu,pl,cardio){return[
 {name:'Strength Trial',focus:'Strength / Core',time:(cardio+10)+'–'+(cardio+18)+' min',tasks:[task(sq+' squats',30,'POWER STRIKE'),task(pu+' push-ups',30,'POWER STRIKE'),task(lu+' alternating lunges',15,'LEG BREAK'),task(pl+'-second plank',15,'CORE BREAK'),task(cardio+'-minute cardio',10,'ENDURANCE HIT')]},
 {name:'Endurance Trial',focus:'Endurance / Conditioning',time:(cardio+12)+'–'+(cardio+20)+' min',tasks:[task(Math.max(10,sq-5)+' squats',10,'POWER HIT'),task(Math.max(5,pu-3)+' push-ups',10,'POWER HIT'),task((lu+6)+' step-ups',20,'ENDURANCE STRIKE'),task(Math.round(pl*1.15)+'-second plank',20,'CORE BREAK'),task((cardio+3)+'-minute cardio',40,'ENDURANCE CRITICAL')]},
 {name:'Full Body Trial',focus:'Full Body / Control',time:(cardio+10)+'–'+(cardio+18)+' min',tasks:[task(sq+' chair or bodyweight squats',20,'STRIKE'),task(pu+' incline or standard push-ups',20,'STRIKE'),task(lu+' reverse lunges',20,'STRIKE'),task(Math.max(10,Math.round(sq/2))+' glute bridges',20,'STRIKE'),task(cardio+'-minute cardio',20,'STRIKE')]},
 {name:'System Circuit',focus:'Circuit / Conditioning',time:(cardio+8)+'–'+(cardio+16)+' min',tasks:[task('3 rounds: '+Math.ceil(sq/3)+' squats',15,'COMBO HIT'),task('3 rounds: '+Math.ceil(pu/3)+' push-ups',20,'COMBO HIT'),task('3 rounds: '+Math.ceil(lu/3)+' alternating lunges',25,'HEAVY COMBO'),task('3 rounds: '+Math.ceil(pl/3)+'-second plank',15,'CORE BREAK'),task(Math.max(5,cardio-5)+'-minute cardio finisher',25,'FINISHER')]}
]}
function task(text,damage,attack){return{text,damage,attack}}
function normalizeVariant(v){v.tasks=v.tasks.map((t,i)=>typeof t==='string'?task(t,20,'SYSTEM STRIKE'):t);return v}
BOSS_STAGES.forEach(b=>b.variants.forEach(normalizeVariant));
/* D-rank gets hand-tuned damage to match each trial focus. */
Object.assign(BOSS_STAGES[0].variants[0],{tasks:[task('20 bodyweight squats',30,'POWER STRIKE'),task('10 push-ups (modified allowed)',30,'POWER STRIKE'),task('20 alternating lunges',15,'LEG BREAK'),task('30-second plank',15,'CORE BREAK'),task('10-minute walk or cardio',10,'ENDURANCE HIT')]});
Object.assign(BOSS_STAGES[0].variants[1],{tasks:[task('15 bodyweight squats',10,'POWER HIT'),task('8 push-ups (modified allowed)',10,'POWER HIT'),task('30 step-ups',20,'ENDURANCE STRIKE'),task('45-second plank',20,'CORE BREAK'),task('12-minute brisk walk',40,'ENDURANCE CRITICAL')]});
Object.assign(BOSS_STAGES[0].variants[2],{tasks:[task('20 chair squats',20,'STRIKE'),task('12 incline push-ups',20,'STRIKE'),task('20 reverse lunges',20,'STRIKE'),task('10 glute bridges',20,'STRIKE'),task('10-minute cardio',20,'STRIKE')]});
Object.assign(BOSS_STAGES[0].variants[3],{tasks:[task('3 rounds: 10 squats',15,'COMBO HIT'),task('3 rounds: 5 push-ups',20,'COMBO HIT'),task('3 rounds: 10 alternating lunges',25,'HEAVY COMBO'),task('3 rounds: 20-second plank',15,'CORE BREAK'),task('5-minute cardio finisher',25,'FINISHER')]});,
...
function renderSideSystem(){
 ...
 wrap.innerHTML=BOSS_STAGES.map(b=>{const e=s.boss[b.rank]||{attempts:0,history:[]},passed=!!e.passed,eligible=bossEligible(b,s),active=e.activeVariant!=null?b.variants[e.activeVariant]:null,best=(e.history||[]).reduce((m,x)=>Math.max(m,x.completed||0),0);
  ...
  if(!active)return '<article class="boss-card boss-ready"><div class="boss-head"><div><span class="side-tag">BOSS INTEL</span><h3>'+b.title+'</h3><p>Level '+b.level+' • +'+b.reward+' XP • '+b.variants.length+' possible trials</p><p>Previous Attempts: '+(e.attempts||0)+' • Best Result: '+best+'/5</p><p>Exact objectives and order remain classified until battle begins.</p></div><strong>'+b.rank+'</strong></div><button type="button" data-start="'+b.rank+'">BEGIN RANK TRIAL</button></article>';
  const order=e.order||active.tasks.map((_,i)=>i),pos=Math.min(e.current||0,order.length-1),obj=active.tasks[order[pos]],hp=Math.max(0,100-(e.damage||0));
  return '<article class="boss-card boss-ready boss-battle"><div class="boss-head"><div><span class="side-tag">ACTIVE BOSS: '+active.name+'</span><h3>'+b.title+'</h3><p>'+active.focus+' • '+active.time+' • Attempt '+((e.attempts||0)+1)+'</p></div><strong>'+b.rank+'</strong></div><div class="boss-hp"><div class="boss-hp__meta"><span>BOSS HP</span><strong>'+hp+' / 100</strong></div><div class="boss-hp__track"><i style="width:'+hp+'%"></i></div></div><div class="boss-objective"><span class="side-tag">OBJECTIVE '+((e.current||0)+1)+'/'+order.length+' REVEALED</span><h3>'+obj.text+'</h3><p>Potential Damage: '+obj.damage+' • '+obj.attack+'</p></div><button type="button" data-hit="'+b.rank+'">COMPLETE OBJECTIVE</button><button type="button" data-end="'+b.rank+'">END ATTEMPT</button></article>';
 }).join('');
 ...
 wrap.querySelectorAll('[data-hit]').forEach(x=>x.onclick=()=>completeBossObjective(x.dataset.hit));
...
function completeBossObjective(rank){const b=BOSS_STAGES.find(x=>x.rank===rank),s=loadSideSystem(),e=s.boss[rank];if(!b||!e||e.activeVariant==null)return;const v=b.variants[e.activeVariant],order=e.order||v.tasks.map((_,i)=>i),pos=e.current||0,obj=v.tasks[order[pos]];e.damage=(e.damage||0)+obj.damage;e.checks=e.checks||[];e.checks[order[pos]]=true;e.current=pos+1;s.boss[rank]=e;saveSideSystem(s);if(typeof addSystemMessage==='function')addSystemMessage(obj.attack+' — '+obj.damage+' DAMAGE! Boss HP: '+Math.max(0,100-e.damage)+'/100','quest');if(e.current>=order.length){defeatBoss(rank);return}renderSideSystem()}
function endBossAttempt(rank){const b=BOSS_STAGES.find(x=>x.rank===rank),s=loadSideSystem(),e=s.boss[rank];if(!b||!e||e.activeVariant==null)return;const completed=(e.checks||[]).filter(Boolean).length;e.attempts=(e.attempts||0)+1;e.history=e.history||[];e.history.push({variant:e.activeVariant,completed,total:b.variants[e.activeVariant].tasks.length,date:new Date().toISOString(),passed:false});e.lastVariant=e.activeVariant;e.activeVariant=null;e.order=[];e.current=0;e.damage=0;e.checks=[];saveSideSystem(s);if(typeof addSystemMessage==='function')addSystemMessage('BOSS BATTLE ENDED — '+completed+'/5 objectives complete. Next attempt will change.','quest');renderSideSystem()}
function defeatBoss(rank){const b=BOSS_STAGES.find(x=>x.rank===rank),s=loadSideSystem(),e=s.boss[rank];if(!b||!e||e.activeVariant==null)return;const total=b.variants[e.activeVariant].tasks.length,completed=(e.checks||[]).filter(Boolean).length;if(completed<total)returne.attempts=(e.attempts||0)+1;e.history=e.history||[];e.history.push({variant:e.activeVariant,completed,total,date:new Date().toISOString(),passed:true});e.lastVariant=e.activeVariant;e.activeVariant=null;e.checks=[];e.passed=true;e.passedAt=new Date().toISOString();s.boss[rank]=e;saveSideSystem(s);if(typeof addXp==='function')addXp(b.reward,'boss-stage');if(typeof addSystemMessage==='function')addSystemMessage('BOSS DEFEATED! RANK PROMOTION: '+rank+'-CLASS — +'+b.reward+' XP','level');if(typeof saveState==='function')saveState();if(typeof renderAll==='function')renderAll();renderSideSystem();alert('BOSS DEFEATED\nRANK PROMOTION ACHIEVED\n'+rank+'-CLASS\n+'+b.reward+' XP')}
