...
const RANK_STANDARDS={D:{Strength:10,Endurance:10,Conditioning:10,Mobility:8,Consistency:10,Recovery:8},C:{Strength:20,Endurance:20,Conditioning:20,Mobility:16,Consistency:20,Recovery:16},B:{Strength:35,Endurance:35,Conditioning:35,Mobility:30,Consistency:40,Recovery:30},A:{Strength:50,Endurance:50,Conditioning:50,Mobility:42,Consistency:55,Recovery:42},S:{Strength:65,Endurance:65,Conditioning:65,Mobility:55,Consistency:70,Recovery:55},'S+':{Strength:78,Endurance:78,Conditioning:78,Mobility:68,Consistency:82,Recovery:68},Shadow:{Strength:88,Endurance:88,Conditioning:88,Mobility:80,Consistency:92,Recovery:80}};
...
window.SystemBuild={awardTraining:awardBuildTraining,awardMission:awardBuildMission,getBuild:loadBuild,setPath:setTrainingPath};
...
function loadSideSystem(){let s={};try{s=JSON.parse(localStorage.getItem(SIDE_SYSTEM_KEY)||'{}')}catch(e){}if(s.date!==today())s={...s,date:today(),completed:[],adaptiveDone:[]};s.completed=s.completed||[];s.adaptiveDone=s.adaptiveDone||[];s.boss=s.boss||{};return s}
...
function adaptiveMissions(){const b=loadBuild(),s=loadSideSystem(),next=nextPromotion(s),weak=next?weakestStats(next.rank)[0]:null,path=TRAINING_PATHS[b.path]||TRAINING_PATHS.Balanced,out=[];
 const pathMap={'Fat Loss':['PATH MISSION','Conditioning Drive','Complete 25–30 minutes of moderate cardio or conditioning.','Conditioning'],'Muscle Building':['PATH MISSION','Hypertrophy Protocol','Complete a resistance session with controlled working sets.','Strength'],Strength:['PATH MISSION','Power Development','Complete a strength-focused resistance session.','Strength'],Endurance:['PATH MISSION','Endurance Base','Complete 25+ minutes of continuous endurance work.','Endurance'],Balanced:['PATH MISSION','Balanced Development','Complete a full-body session combining strength and conditioning.','Consistency']};
 const p=pathMap[b.path]||pathMap.Balanced;out.push({id:'adaptive_path',tag:p[0],title:p[1],desc:p[2],stat:p[3],xp:60});
 if(weak){const d={Strength:'Complete a strength-focused session.',Endurance:'Complete 20+ minutes of steady endurance work.',Conditioning:'Complete a full-body conditioning circuit.',Mobility:'Complete 15 minutes of mobility work.',Consistency:'Complete today’s scheduled training.',Recovery:'Complete 10–15 minutes of recovery or mobility work.'};out.push({id:'adaptive_build',tag:'BUILD MISSION',title:'Develop '+weak.stat,desc:d[weak.stat],stat:weak.stat,xp:50});out.push({id:'adaptive_promo',tag:'PROMOTION MISSION',title:next.rank+'-Class Preparation',desc:'Raise '+weak.stat+' toward '+weak.need+' to satisfy the next promotion standard.',stat:weak.stat,xp:70})}
 const rec=b.stats.Recovery<=Math.min(...BUILD_STATS.filter(x=>x!=='Recovery').map(x=>b.stats[x]));if(rec)out.push({id:'adaptive_recovery',tag:'RECOVERY DIRECTIVE',title:'Recovery Protocol',desc:'Complete light mobility or recovery work. More punishment is not always more progress.',stat:'Recovery',xp:35});
...
function awakeningClass(scores){const core=['Strength','Endurance','Conditioning','Mobility','Recovery'],avg=Math.round(core.reduce((n,x)=>n+scores[x],0)/core.length),low=Math.min(...core.map(x=>scores[x]));return avg>=70&&low>=50?'C':avg>=45&&low>=30?'D':'E'}
...
function showAwakeningAssessment(done){
 ...
 let step=0,data={sit:0,push:0,walk:0,march:0,mobility:2,energy:2};
 const screens=[
  ()=>'<span class="side-tag">THE AWAKENING TRIAL • 1/5</span><h1>STRENGTH</h1><p>Perform controlled chair sit-to-stands for 60 seconds, then a comfortable push-up variation. Stop before form breaks down.</p><label class="ob-days">Sit-to-stands completed <input id="awSit" type="number" min="0" max="60" value="'+data.sit+'"></label><label class="ob-days">Push-up variation reps <input id="awPush" type="number" min="0" max="50" value="'+data.push+'"></label>',
  ()=>'<span class="side-tag">THE AWAKENING TRIAL • 2/5</span><h1>ENDURANCE</h1><p>Walk continuously for up to 6 minutes at a comfortable purposeful pace. Enter the minutes you completed without needing to stop.</p><label class="ob-days">Minutes completed <input id="awWalk" type="number" min="0" max="6" step=".5" value="'+data.walk+'"></label>',
  ()=>'<span class="side-tag">THE AWAKENING TRIAL • 3/5</span><h1>CONDITIONING</h1><p>March in place at a steady controlled pace for up to 3 minutes. This is not an all-out test.</p><label class="ob-days">Minutes completed <input id="awMarch" type="number" min="0" max="3" step=".5" value="'+data.march+'"></label>',
  ...
  ()=>{const scores=calculate();const rank=awakeningClass(scores);return '<span class="side-tag">THE AWAKENING TRIAL • 5/5</span><h1>CLASSIFICATION COMPLETE</h1><div class="ob-awaken"><small>INITIAL CLASSIFICATION</small><strong>'+rank+'-CLASS</strong><span>MAXIMUM INITIAL CLASS: C</span></div><div class="assessment-list">'+['Strength','Endurance','Conditioning','Mobility','Recovery'].map(x=>'<p><b>'+x+':</b> '+scores[x]+'</p>').join('')+'<p><b>Consistency:</b> 0 • Earned inside The System</p></div><p>Recommended Path: <strong>'+awakeningPath(scores)+'</strong></p>'}
 ];
 ...
 function calculate(){return {Strength:Math.round((awakeningScore(data.sit,30)+awakeningScore(data.push,20))/2),Endurance:awakeningScore(data.walk,6),Conditioning:awakeningScore(data.march,3),Mobility:{1:25,2:55,3:80}[data.mobility]||55,Consistency:0,Recovery:{1:25,2:55,3:80}[data.energy]||55}}
 function finish(){const scores=calculate(),rank=awakeningClass(scores),b=loadBuild();BUILD_STATS.forEach(x=>b.stats[x]=scores[x]);b.path=awakeningPath(scores);b.assessment={version:1,date:new Date().toISOString(),scores:{...scores},class:rank,raw:{...data}};saveBuild(b);applyAssessmentPlacement(rank);localStorage.setItem(ASSESSMENT_KEY,JSON.stringify({rank,scores,raw:data,recommendedPath:b.path,bonus:ASSESSMENT_BONUS[rank]||0,completedAt:new Date().toISOString()}));el.classList.remove('active');if(typeof addSystemMessage==='function')addSystemMessage('THE AWAKENING TRIAL COMPLETE — '+rank+'-CLASS • Previous training recognized.','level');if(typeof renderAll==='function')renderAll();renderSideSystem();if(done)done(rank)}
 function render(){el.innerHTML='<div class="ob-card"><p><b>Safety:</b> Use a stable chair and clear space. Stop immediately for pain, dizziness, chest pain, faintness, or unusual shortness of breath.</p>'+screens[step]()+'<div class="ob-actions">'+(step?'<button id="awBack">BACK</button>':'')+'<button id="awNext" class="btn-primary">'+(step===4?'ACCEPT CLASSIFICATION':'CONTINUE')+'</button></div></div>';el.classList.add('active');el.querySelectorAll('[data-mob]').forEach(x=>x.onclick=()=>{data.mobility=Number(x.dataset.mob);render()});el.querySelectorAll('[data-energy]').forEach(x=>x.onclick=()=>{data.energy=Number(x.dataset.energy);render()});document.getElementById('awBack')?.addEventListener('click',()=>{capture();step--;render()});document.getElementById('awNext').onclick=()=>{capture();if(step<4){step++;render()}else finish()}}
 render();
}
function showOnboarding(){if(!shouldOnboard())return;let el=document.getElementById('systemOnboarding');if(!el){el=document.createElement('div');el.id='systemOnboarding';el.className='system-onboarding';document.body.appendChild(el)}let step=0,data={name:'',goal:'balanced',experience:'beginner',equipment:[],days:4};const screens=[
 ...
 ()=>'<span class="side-tag">EQUIPMENT • 4/4</span><h1>PREPARE FOR ASSESSMENT</h1><p>Select your available gear and weekly training availability.</p><div class="ob-options ob-equipment">'+['No Equipment','Bodyweight','Dumbbells','Barbell','Resistance Bands','Cardio Machine','Full Gym'].map(x=>'<button data-eq="'+x+'" class="'+(data.equipment.includes(x)?'selected':'')+'">'+x+'</button>').join('')+'</div><label class="ob-days">Training days per week <input id="obDays" type="number" min="1" max="7" value="'+data.days+'"></label>'
 ];function render(){el.innerHTML='<div class="ob-card">'+screens[step]()+'<div class="ob-actions">'+(step?'<button id="obBack">BACK</button>':'')+'<button id="obNext" class="btn-primary">'+(step===3?'BEGIN ASSESSMENT':'CONTINUE')+'</button></div></div>';el.classList.add('active');el.querySelectorAll('[data-goal]').forEach(x=>x.onclick=()=>{data.goal=x.dataset.goal;render()});el.querySelectorAll('[data-exp]').forEach(x=>x.onclick=()=>{data.experience=x.dataset.exp;render()});el.querySelectorAll('[data-eq]').forEach(x=>x.onclick=()=>{data.equipment=data.equipment.includes(x.dataset.eq)?data.equipment.filter(v=>v!==x.dataset.eq):[...data.equipment,x.dataset.eq];render()});document.getElementById('obBack')?.addEventListener('click',()=>{step--;render()});document.getElementById('obNext').onclick=()=>{if(step===0)data.name=(document.getElementById('obName')?.value||'').trim()||'Player';if(step===3)data.days=Math.max(1,Math.min(7,Number(document.getElementById('obDays')?.value)||4));if(step<3){step++;render();return}const path=onboardingPath(data.goal),b=loadBuild();b.path=path;b.profile={experience:data.experience,equipment:data.equipment.length?data.equipment:['No Equipment'],days:data.days};saveBuild(b);localStorage.setItem(ONBOARDING_KEY,JSON.stringify({...data,path,assessmentPending:true,completedAt:new Date().toISOString()}));el.classList.remove('active');showAwakeningAssessment(()=>{const ob=onboardingData()||{};ob.assessmentPending=false;localStorage.setItem(ONBOARDING_KEY,JSON.stringify(ob));renderPlayerStatus()})}}render()}
